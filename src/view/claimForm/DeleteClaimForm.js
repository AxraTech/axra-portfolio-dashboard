...
import { CLAIM_FORM_PK, DELETE_CLAIM_FORM } from "../../gql/claimForm";
...
const DeleteLeaveForm = () => {
  ...
  const { data: home } = useQuery(CLAIM_FORM_PK, {
    variables: { id: id },
  });
  const [delete_home] = useMutation(DELETE_CLAIM_FORM, {
    onError: (err) => {
  ...
  const handleDelete = async () => {
    try {
      await delete_home({
        variables: { id: home?.claim_form_by_pk.id },
      });
    } catch (err) {
