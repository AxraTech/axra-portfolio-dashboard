...
import { ADD_SERVICE_CATEGORY } from "../../gql/serviceCategory";
...
const CreateServiceCategory = () => {
  ...
  //create products
  const [add_service] = useMutation(ADD_SERVICE_CATEGORY, {
    onError: (err) => {
  ...
  const handleCreate = async (e) => {
    ...
    const tempErrors = {};
    if (!values.image_url) {
      tempErrors.image_url = "service Image field is required.";
      errorExist = true;
    }

    if (!values.service_name) {
      tempErrors.service_name = "service Brand field is required.";
      errorExist = true;
    }
