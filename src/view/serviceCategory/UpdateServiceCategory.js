...
const UpdateProductBrand = () => {
  ...
  const [loadService, resultService] = useLazyQuery(SERVICE_CATEGORY_BY_PK);

  useEffect(() => {
    loadService({ variables: { id: id } });
  }, [loadService]);
  ...
  // Create products
  const [edit_product] = useMutation(UPDATE_SERVICE_CATEGORY, {
    onError: (err) => {
