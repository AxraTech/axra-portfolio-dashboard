...
import { GET_ALL_APPOINMENT_FORM } from "../../gql/userAppointmentForm";
...
const Appointments = () => {
  ...
  const [loadForm, resForm] = useLazyQuery(GET_ALL_APPOINMENT_FORM);
  ...
  useEffect(() => {
    if (resForm.data) {
      setHome(resForm?.data.user_appointment_form);
    }
  }, [resForm]);
  ...
  const handleRemove = (row) => {
    navigate(`/deleteAppointment/${row.id}`);
  };
              ...
              {Array.isArray(currentData) &&
                currentData.map((row, index) => (
                  ...
                  >
                    ...
                    <td className="py-4">{row?.company_name}</td>
