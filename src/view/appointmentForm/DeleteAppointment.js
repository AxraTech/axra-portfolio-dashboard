...
const DeleteLeaveForm = () => {
  ...
  const { data: home } = useQuery(APPOINTMENT_BY_PK, {
    variables: { id: id },
  });
  const [delete_home] = useMutation(DELETE_APPOINTMENT, {
    onError: (err) => {
  ...
  const handleDelete = async () => {
    try {
      await delete_home({
        variables: { id: home?.user_appointment_form[0].id },
      });
    } catch (err) {
