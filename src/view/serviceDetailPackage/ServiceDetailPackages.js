...
import { ALL_SERVICE_DETAILS_PACKAGES } from "../../gql/serviceDetailPackage";

const ServiceDetailPackages = ({ serviceId }) => {
  ...
  const [loadService, resultService] = useLazyQuery(
    ALL_SERVICE_DETAILS_PACKAGES
  );

  useEffect(() => {
    loadService({
      variables: { fk_service_details_id: serviceId },
  ...
  useEffect(() => {
    if (resultService.data) {
      setServices(resultService?.data?.service_details_packages);
    }
  }, [resultService]);

  // Calculate total number of pages
  const totalPages = Math.ceil(
    resultService?.data?.service_details_packages_aggregate?.aggregate.count /
      itemsPerPage
  );
  const totals =
    resultService?.data?.service_details_packages_aggregate?.aggregate.count;
            ...
            {Array.isArray(currentData) &&
              currentData.map((row, index) => (
                ...
                >
                  ...
                  <td className="pr-2">
                    {row?.service_details_packages?.service_category?.service_name.substring(
                      0,
                      15
                    )}
                  </td>
                  <td className="pr-2">
                    {row?.service_packages_details?.service_package_type?.substring(
                      0,
                      10
                    )}
                  </td>
                  <td className="pr-2">
                    {row?.service_packages_details?.service_package_name?.substring(
                      0,
                      10
                    )}
                  </td>
                  ...
                  <td className="pr-2">
                    {row?.service_packages_details?.one_time_package_price}
                  </td>
                  <td className="pr-2">
                    {row?.service_packages_details?.recurrently_service_fee?.substring(
                      0,
                      10
                    )}
                  </td>
                  ...
                  <td className="pr-2 ">
                    <button
                      onClick={() =>
                        navigate(
                          `/service_package/${row?.service_packages_details?.id}`
                        )
                      }
...
export default ServiceDetailPackages;
