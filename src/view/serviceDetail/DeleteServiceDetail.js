...
const DeleteServiceDetail = ({ details, detailId, handleDelClose }) => {
  const navigate = useNavigate();
  const [loading, setLoading] = useState(false);
  const [delete_serviceDetail] = useMutation(DELETE_SERVICE_DETAIL, {
    onError: (err) => {
    ...
    try {
      if (detailId) {
        await delete_serviceDetail({
          variables: { id: detailId?.service_details_by_pk?.id },
        });
      } else {
        await delete_serviceDetail({
