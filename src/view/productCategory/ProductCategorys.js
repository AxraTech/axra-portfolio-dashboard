...
const ProductCategorys = () => {
  ...
  const [loadService, resultProduct] = useLazyQuery(ALL_PRODUCT_CAT);
  ...
  useEffect(() => {
    ...
    setNav("product_category");
  }, [loadService, searchValue, nav]);
  useEffect(() => {
    if (resultProduct.data) {
      setProduct(resultProduct?.data.product_category);
    }
  }, [resultProduct]);
  ...
  const handleRemove = (row) => {
    navigate(`/delete_product_cat/${row.id}`);
  };
  if (!product) {
    return;
  }
      ...
      <div className="flex justify-between mb-3 ">
        ...
        <div className="flex items-center">
          <button
            ...
            onClick={() => navigate("/create_product_cat")}
          >
            <svg
              ...
              {Array.isArray(currentData) &&
                currentData.map((row, index) => (
                  ...
                  >
                    ...
                    <td className="py-4">{row?.product_category_name}</td>
