...
const UpdateProductBrand = () => {
  ...
  const [loadProduct, resultProduct] = useLazyQuery(PRODUCT_BRAND_PK);
  ...
  useEffect(() => {
    if (resultProduct.data) {
      setValues({
        id: resultProduct.data.product_brand_by_pk.id ?? "",
        brand_name: resultProduct.data.product_brand_by_pk.brand_name ?? "",

        image_url: resultProduct.data.product_brand_by_pk.image_url ?? "",
      });
    }
  }, [resultProduct]);
  ...
  // Create products
  const [add_product] = useMutation(EDIT_PRODCUT_BRAND, {
    onError: (err) => {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      await add_product({ variables: updatedValues });
