...
import { DELETE_PRODCUT_BRAND, PRODUCT_BRAND_PK } from "../../gql/productBrand";
...
const DeleteProductBrand = () => {
  ...
  const [delete_serviceDetail] = useMutation(DELETE_PRODCUT_BRAND, {
    onError: (err) => {
  ...
  const handleDelete = async () => {
    try {
      await delete_serviceDetail({
        variables: { id: product?.product_brand_by_pk.id },
      });
    } catch (err) {
