...
const CreateWebProject = () => {
  ...
  //create app projects
  const [add_web] = useMutation(ADD_WEB_PROJECT, {
    onError: (err) => {
      console.log("Project upload error", err);
      alert("Project Upload Error");
      setLoading(false);
    },
    onCompleted: (data) => {
      alert("New Website Project has been added");
  ...
  const handleCreate = async (e) => {
    ...
    if (!values.website_url) {
      tempErrors.website_url = "Website Url field is required.";
      errorExist = true;
    }
      ...
      <div className="grid grid-cols-4 gap-x-10">
        ...
        <form onSubmit={handleCreate} className="col-span-2">
          ...
          <div>
            ...
            {errors.website_url && (
              <p className="text-red-500 mt-2 text-sm">{errors.website_url}</p>
            )}
          </div>
        </form>
      </div>
